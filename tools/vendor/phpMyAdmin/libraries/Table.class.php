...
class PMA_Table
{
    ...
    static function generateFieldSpec($name, $type, $index, $length = '',
        ...
        $color_field_name = ($C_cls && !empty($_REQUEST['color_field_name'][$index])) ? $_REQUEST['color_field_name'][$index] : '';
        $color_field_type = ($C_cls && !empty($_REQUEST['color_field_type'][$index])) ? $_REQUEST['color_field_type'][$index] : '';
        $query = $color_field_name.PMA_Util::backquote($name).$C_cls . ' ' . $color_field_name.$type.$C_cls;
        ...
        if ($length != '' && ! preg_match($pattern, $type)) {
            $color_field_length = ($C_cls && !empty($_REQUEST['color_field_length'][$index])) ? $_REQUEST['color_field_length'][$index] : '';
            $query .= '(' . $color_field_length.$length.$C_cls . ')';
        }

        if ($attribute != '') {
            $color_field_attribute = ($C_cls && !empty($_REQUEST['color_field_attribute'][$index])) ? $_REQUEST['color_field_attribute'][$index] : '';
            $query .= ' ' . $color_field_attribute.$attribute.$C_cls;
        }
        ...
        if (! empty($collation) && $collation != 'NULL' && $matches) {
            $color_field_collation = ($C_cls && !empty($_REQUEST['color_field_collation'][$index])) ? $_REQUEST['color_field_collation'][$index] : '';
            $query .= $color_field_collation.PMA_generateCharsetQueryPart($collation).$C_cls;
        }

        if ($null !== false) {
            $color_field_null = ($C_cls && !empty($_REQUEST['color_field_null'][$index])) ? $_REQUEST['color_field_null'][$index] : '';
            $query .= $color_field_null;
            if ($null == 'NULL') {
                $query .= ' NULL';
            } else {
                $query .= ' NOT NULL';
        ...
        $color_field_default_value = ($C_cls && !empty($_REQUEST['color_field_default_value'][$index])) ? $_REQUEST['color_field_default_type'][$index] : '';
        $query .= $color_field_default_value;
        switch ($default_type) {
        case 'USER_DEFINED' :
        ...
        case 'CURRENT_TIMESTAMP' :
            $color_field_default_type = ($C_cls && !empty($_REQUEST['color_field_default_type'][$index])) ? $_REQUEST['color_field_default_type'][$index] : '';
            $query .= ' DEFAULT ' . $color_field_default_type.$default_type.$C_cls;
            break;
        case 'NONE' :
        default :
            break;
        ...
        if (!empty($extra)) {
            $color_field_extra = ($C_cls && !empty($_REQUEST['color_field_extra'][$index])) ? $_REQUEST['color_field_extra'][$index] : '';
            $query .= ' ' . $color_field_extra.$extra.$C_cls;
        }
        if (!empty($comment)) {
            $color_field_comments = ($C_cls && !empty($_REQUEST['color_field_comments'][$index])) ? $_REQUEST['color_field_comments'][$index] : '';
            $query .= " COMMENT '" . $color_field_comments.PMA_Util::sqlAddSlashes($comment).$C_cls . "'";
        }
